...
turn on 222,12 through 856,241";
 ...
 foreach(string cmd in input.Split("\n"))
 {
   ...
   string[] num = parts[parts.Length -1].Split(",");
   Vector2 endPos = new Vector2(int.Parse(num[0]),int.Parse(num[1]));
   num = parts[parts.Length - 3].Split(",");
   Vector2 startPos = new Vector2(int.Parse(num[0]),int.Parse(num[1]));
   if(cmd.Contains("on")){board.TurnOn(startPos, endPos);}
   else if(cmd.Contains("off")){board.TurnOff(startPos, endPos);}
   else{board.Toggle(startPos, endPos);}
...
class Board
 {
    ...
    public Light[] lights = new Light[size.x * size.y];
    public Board() 
    {
    ...
    public void TurnOn(Vector2 startPos, Vector2 endPos) 
    {
        for(int x = startPos.x; x <= endPos.x; x++)
        {
            for(int y = startPos.y; y <= endPos.y; y++)
            {
                lights[y * size.x + x].TurnOn();
    ...
    public void TurnOff(Vector2 startPos, Vector2 endPos) 
    {
        for(int x = startPos.x; x <= endPos.x; x++)
        {
            for(int y = startPos.y; y <= endPos.y; y++)
            {
                lights[y * size.x + x].TurnOff();
            }
        }
    }
    public void Toggle(Vector2 startPos, Vector2 endPos) 
    {
        for(int x = startPos.x; x <= endPos.x; x++)
        {
            for(int y = startPos.y; y <= endPos.y; y++)
            {
                lights[y * size.x + x].Toggle();
            }
